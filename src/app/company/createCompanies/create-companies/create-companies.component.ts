...
export class CreateCompaniesComponent implements OnInit {
    ...
    this.companiesCreated=({
      ...
      this._companyService
        ...
        .subscribe(companie => {
          ...
          console.log(this.idCompaniesVar);
          this.hasCreated=({
  ...
  joinCompanie() {
    ...
    this._companyService.join(dtoJoin)
      .subscribe(()=>{this._companyService.fetchByName(dtoJoin.name)
        .subscribe(companie=>{this.idCompaniesVar=companie[0].idCompanies;this.hasCreated=({
          idCompanies:this.idCompaniesVar,
          idFunctions:2,
          idAccount:Number(this._session.getID()),
          idHas:0,
